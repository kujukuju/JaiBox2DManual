...
float WeldJoint_get_reference_angle(const b2WeldJoint* self) {
    return self->GetReferenceAngle();
}
void WeldJoint_set_frequency(b2WeldJoint* self, float frequency) {
    self->SetFrequency(frequency);
}
float WeldJoint_get_frequency(const b2WeldJoint* self) {
    return self->GetFrequency();
}
void WeldJoint_set_damping_ratio(b2WeldJoint* self, float ratio) {
    self->SetDampingRatio(ratio);
}
float WeldJoint_get_damping_ratio(const b2WeldJoint* self) {
